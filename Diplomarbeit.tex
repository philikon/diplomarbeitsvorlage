%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Diplomarbeitsvorlage von Hartmut Stöcker, Stand 25.01.2008
% Modifiziert von Philipp v. Weitershausen im April 2009


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Optionen für die Dokumentklasse scrbook aus dem KOMA-Skript:
% - 12pt: Schriftgröße 12 pt
% - twoside: Doppelseitiger Ausdruck
% - BCOR20mm: Bindekorrektur 20mm
% - DIV18: äußerer Rand ca. 20mm (damit innerer Rand + BCOR = 30mm)
% - headinclude: Festlegung der Seitenränder unter Einbeziehung der Kopfzeile
% - liststotoc: Abbildungs- und Tabellenverzeichnis ins Inhaltsverzeichnis
% - bibtotoc: Literaturverzeichnis ins Inhaltsverzeichnis aufnehmen
% - tablecaptionabove: Tabellenbeschriftung über der Tabelle
% - ngerman: Neu-deutsche Silbentrennung

\documentclass[12pt,twoside,BCOR20mm,DIV18,headinclude,headsepline,liststotoc,bibtotoc,tablecaptionabove,ngerman]{scrbook}

% Laden der benötigten Pakete
\usepackage{graphicx}             % Grafiken einfügen
\usepackage{subfigure}            % Teilbilder erzeugen
\usepackage{amsmath}              % Mathematikbefehle
\usepackage{amssymb}              % Unter anderem schöne Mengensymbole
\usepackage[amssymb]{SIunits}     % Einheiten per Befehl einfügen
\usepackage{multirow}             % Tabellenzeilen verbinden
\usepackage{tabularx}             % Tabellen mit fester Breite

% Benutze Times as Serif-Schrift, Helvetica als Sans-Serif-Schrift.
% Ist angenehmer zu lesen als Computer Modern (LaTeXs Standard-
% Serif-Schrift) und sorgt nebenbei für kompaktere Postscript-
% bzw. PDF-Dateien.
\usepackage{times}

% PDF-Dokumenteigenschaften setzen, PDF-Lesezeichen erzeugen, Verweise
% anklickbar machen
\usepackage[pdftitle={Diplomarbeit}, pdfauthor={Vorname Name}, bookmarks=true, pdfborder={0 0 0}]{hyperref}
% Verweise nicht auf Bildunterschrift, sondern auf Bild selbst
\usepackage[all]{hypcap}

% Bildunterschrift 0.8 cm einrücken, Größe small, Abbildung wird fett
% geschrieben
\usepackage[margin=0.8cm, font=small, labelfont=bf]{caption}

% Deutsche Zeichen- und Sprachpakete
% Unter Windows ist 'latin1' durch 'ansinew' zu ersetzen
\usepackage[latin1]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[ngerman]{babel}

% Keine Einrückung am Absatzanfang
\setlength{\parindent}{0mm}

% 1,5 facher Zeilenabstand
\usepackage{setspace}
\onehalfspacing

% Entferne die Kommentarzeichen der folgenden Zeilen, um Formel-,
% Bilder- und Tabellennummer mit kapitel.abschnitt.lfd-nr zu
% nummerieren.  Ansonsten wird die Standardeinstellung kapitel.lfd-nr
% verwendet.
%\numberwithin{equation}{section}
%\numberwithin{figure}{section}
%\numberwithin{table}{section}

% Kopfzeile mit Kapitelname auf der linken, Abschnittsname auf der
% rechten Seite.  Fußzeile mit Seitennummer.
\usepackage{scrpage2}
\pagestyle{scrheadings}
\automark[section]{chapter}

% Erfahrungswerte für Parameter der automatischen Silbentrennung
\hyphenpenalty=500          % vermeide zu häufige Trennung
\doublehyphendemerits=9000  % vermeide Trennung in aufeinander folgenden Zeilen
\finalhyphendemerits=6000

% Feynman-Diagramme mit LaTeX und MetaPost zeichnen.
\usepackage{feynmp}
\DeclareGraphicsRule{*}{mps}{*}{}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Anfang des Dokuments

\begin{document}

% Seiten bis zum Beginn des eigentlichen Textes mit römischen
% Seitenzahlen in der Fußzeile
\frontmatter
\pagestyle{plain}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Titelseite

\thispagestyle{empty}

\vspace{-5em}
\begin{flushleft}\includegraphics[angle=0,width=60mm]{Bilder/TU_Logo_sw}\par\end{flushleft}
\vspace{-3em}
\begin{center}\rule{\textwidth}{0.1ex}\par\end{center}
\vspace{-4em}
\begin{center}\rule{\textwidth}{0.1ex}\par\end{center}

\vfill

\begin{center}\textbf{\Huge Hier kommt}\par\end{center}
\begin{center}\textbf{\Huge der Titel}\par\end{center}
\begin{center}\textbf{\Huge hin}\par\end{center}

\vfill

\begin{center}
{\large Diplomarbeit}\\
{\large zur Erlangung des wissenschaftlichen Grades}\\
{\large Diplom-Physiker}\par
\end{center}

\begin{center}vorgelegt von\par\end{center}
\begin{center}{\large Vorname Name}\\geboren am xx.yy.zzzz in Ort\par\end{center}

\vspace{13mm}

\begin{center}
{\large Institut für ...}\\
{\large der Technischen Universität Dresden}\\
{\large 200x}\par
\end{center}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Gutachter-Seite

\newpage
\thispagestyle{empty}

\ \vfill

\begin{tabular}{l}
Eingereicht am xx.yy.zzzz \\
\end{tabular}

\vspace{1cm}

\begin{tabular}{ll}
1. Gutachter: & ...\\
2. Gutachter: & ...\\
\end{tabular}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Zusammenfassung/Abstract-Seite

\newpage
\thispagestyle{empty}

\section*{\centering{Kurzdarstellung}}

Abstract auf Deutsch...

\vspace{3mm}

\section*{\centering{Abstract}}

Abstract auf Englisch...


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Inhaltsverzeichnis, Abbildungsverzeichnis, Tabellenverzeichnis

\tableofcontents
\listoffigures
\listoftables


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Haupttext

\mainmatter
\pagestyle{scrheadings}         % Oben definierte Kopf- und Fußzeile verwenden

\include{Kapitel/Einleitung}    % Dateien ohne Endung .tex einfügen!
\include{Kapitel/Grundlagen}
\include{Kapitel/Messaufbau}
\include{Kapitel/Messwerte}
\include{Kapitel/Diskussion}
\include{Kapitel/Zusammenfassung}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Anhang

\appendix

\include{Kapitel/Anhang}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Literaturverzeichnis

\backmatter
\newpage
\pagestyle{plain}             % nur Nummerierung in der Fußzeile

\bibliographystyle{alpha}     % Zitierstil: alpha = [Nam88]
\bibliography{Literatur}      % BibTeX-Datei name.bib ohne .bib hier einfügen
%\nocite{*}                    % Listet alle Einträge der Datei auf, wenn aktiv


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Danksagung

\newpage
\thispagestyle{empty}

\section*{\centering{Danksagung}}\bigskip

{\parindent 0mm

Allgemeine Dankesworte...\bigskip

Spezielle Dankesworte...\bigskip

Weitere Dankesworte...

}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Erklärung

\newpage
\thispagestyle{empty}

\section*{\centering{Erklärung}}\bigskip

Hiermit versichere ich, dass ich die vorliegende Arbeit ohne
unzulässige Hilfe Dritter und ohne Benutzung anderer als der
angegebenen Hilfsmittel angefertigt habe. Die aus fremden Quellen
direkt oder indirekt übernommenen Gedanken sind als solche kenntlich
gemacht. Die Arbeit wurde bisher weder im Inland noch im Ausland in
gleicher oder ähnlicher Form einer anderen Prüfungsbehörde vorgelegt.\\[2.3cm]
Vorname Name\\
Dresden, Monat 200x


\end{document}